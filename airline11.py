...
if df.select_dtypes(include=['object']).shape[1] > 0:
    print("Warning: Some columns are still strings!")
df = df.apply(pd.to_numeric, errors='coerce')
df = df[~df.apply(lambda row: row.astype(str).str.strip().eq("").all(), axis=1)]

page = st.sidebar.selectbox("Select a Page", ["Home", "Data Overview", "Exploratory Data Analysis", "Model Triaining and Evaluation", "Make Predictions!", "Extras"])
...
if page == 'Exploratory Data Analysis':
    ...
    if 'Scatterplot' in eda_type:
        st.subheader("Scatterplot - Visualizing Relationships")
...
page = st.selectbox("Choose a Page", ["Overview", "Model Training and Evaluation"], key="page")
...
if page == "Model Training and Evaluation":
    ...
    model_option = st.sidebar.selectbox("Select a model", ["Logistic Regression", "Random Forest"])

    # Prepare the data
    X = df.drop(columns=['satisfaction', 'Type of Travel', 'Class'])
    y = df['satisfaction']
